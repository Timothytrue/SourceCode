...
def load_data(size=1024, filename='9.11usrp_zigbeeonwifi_rxgain10.dat', n=-1):

    # 获取文件的样本数目
    data_count = len(np.fromfile(filename, dtype="float32", count=-1)) 
    
    if(data_count%2==0):
            
        # load size*n IQdata
        IQdata = np.fromfile(filename, dtype="float32", count=data_count)
    else:
        IQdata = np.fromfile(filename, dtype="float32", count=data_count-1)
